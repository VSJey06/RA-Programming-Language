...
class Runtime:
    ...
    def __init__(self, data_dir: str | None = None) -> None:
        ...
        self.db_engine = DatabaseEngine(data_dir)
    ...
    def execute_node(self, node: Node) -> None:
        ...
        elif isinstance(node, AICallNode):
            self._execute_ai_call(node)
        elif isinstance(node, ReturnNode):
            self._execute_return(node)
        elif isinstance(node, DbNextNode):
            self._execute_db_next(node)
        elif isinstance(node, DbBreakNode):
            self._execute_db_break(node)
        else:
    ...
    # ── AI Call / Return / db.next / db.break ────────────────────────────────

    def _execute_ai_call(self, node: AICallNode) -> None:
        """Execute an AI inference call.

        AI is scheduled for v2.0.  Store a placeholder value.
        """
        target = self._locals[-1] if self._locals else self.global_scope
        target[node.var_name] = "[AI unavailable]"
        print(f"AI call to '{node.var_name}' — not available in v1.0")

    def _execute_return(self, node: ReturnNode) -> None:
        """Execute a return statement: evaluate the value, store it, print."""
        value = self.evaluate(node.value)
        self._return_value = value
        print(value)

    def _execute_db_next(self, node: DbNextNode) -> None:
        """Advance the database cursor.

        Cursor iteration is not yet implemented in v1.0.  Currently a no-op.
        """

    def _execute_db_break(self, node: DbBreakNode) -> None:
        """Break out of the database loop.
